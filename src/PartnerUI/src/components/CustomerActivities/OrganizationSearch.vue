...
<script>
import lookupApi from '@/api/crm/callCenter'

export default {
  model: {
    prop: 'internalOrganizationPartyId',
    event: 'save'
  },
  props: {
  ...
  methods: {
    organizationSelected(val) {
      ...
      this.$emit('save', this.internalOrganizationPartyId)
    },
    searcOrganization(query, partyRoleType, partyId) {
      this.busyOrganization = true
      lookupApi.getInternalOrganization(query, this.$store.state.user.sub, partyRoleType, partyId, false, this.hasPortfolio, this.containsItself)
        .then((v) => {
