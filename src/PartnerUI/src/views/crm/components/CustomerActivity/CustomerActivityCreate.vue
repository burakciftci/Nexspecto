      ...
      >
        <el-row>
          <el-col :span="8">
            ...
            >
              <el-col :span="11">
                <el-date-picker
                  :disabled="(!isNew || completeDirectly) && !confirmationCreatedByCC"
        ...
        <el-row>
          <el-collapse class="documentGroupCollapse">
            <el-collapse-item title="Notlar" name="1">
              <el-form-item>
...
export default {
  ...
  methods: {
    ...
    getActivityValues(activityId) {
      if (activityId !== 0) {
        customerActivityApi.getActivityValues(activityId)
          .then((v) => {
            var value = v.data
            this.model = {
              ...
              assignedDate: new Date(value.assignedDateTime),
              assignedHour: new Date(value.assignedDateTime),
    ...
    checkAvailability() {
      customerActivityApi
        .checkAvailability(this.customerId, this.$store.state.user.sub, this.model.customerActivityId)
        .then(v => {
          if (!this.isUserCC) {
